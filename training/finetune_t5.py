...
if (tc == "top"):
    ...
    #train_df = pd.read_json(f"{datadir}/train_{ntc}_plusbv.json")
    #val_df = pd.read_json(f"{datadir}/test_{ntc}_plusbv.json")
    data_list = []
    with open(f"{datadir}/source_data/primevul_train.jsonl", 'r') as file:
        for line in file:
    ...
    data_list = []
    with open(f"{datadir}/source_data/primevul_test.jsonl", 'r') as file:
        for line in file:
            data = json.loads(line)  # Convert JSON string to Python dictionary
            data_list.append(data)  # Append the dictionary to the list
    val_df = pd.DataFrame(data_list)
    train_df = train_df[['func','target']]
    val_df = val_df[['func','target']]
    train_df = train_df.rename(columns={'func':'text','target':'label'})
    val_df = val_df.rename(columns={'func':'text','target':'label'})

    model_dir = "../../selective_model_all"
...
def filter_valid_rows(example):
    """Filter rows with valid text and log the ones that fail."""
    try:
        ...
        # Keep rows with exactly 1 <eos> token
        if eos_count == 1:
            return True  # Row is valid
        else:
            print(f"Filtered out row (invalid eos count): {example}")
            return False  # Row is invalid

    except Exception as e:
        print(f"Error processing row: {example} - Error: {e}")
        return False  # Row is invalid

filtered_datasets = raw_datasets.filter(filter_valid_rows)

# Rename 'label' to 'labels' if needed
if "label" in filtered_datasets["train"].column_names:
    filtered_datasets = filtered_datasets.rename_column("label", "labels")
...
def tokenize_function(examples):
    ...
    tokenized["labels"] = examples["labels"]  # Use renamed column
    return tokenized

tokenized_datasets = filtered_datasets.map(
    tokenize_function,
    batched=True,
    remove_columns=["text"],  # Remove only 'text', keep 'labels'
)

logger.info("Starting tokenization on filtered dataset...")
    ...
    for epoch in range(num_epochs):
        ...
        if accelerator.is_main_process:
            ...
            if eval_metrics['f1'] > best_f1:
                ...
                # Save best model
                if (tc == "top"):
                    checkpoint_prefix = f'../../selective_model_RQ1_PV/{model_checkpoint}_top_{ntc}'
                if (tc == "cwe"):
                    checkpoint_prefix = f'../../selective_model_RQ1_PV/{model_checkpoint}_cwe_{ntc}'
...
if __name__ == "__main__":
    ...
    if ((tc == "top") or (tc == "cwe")):
        if best_model_path:
            ...
            if "label" in all_df.columns and "new_label" in all_df.columns:
                logger.warning("Duplicate 'label' column found. Dropping old 'label' column.")
                all_df = all_df.drop(columns=["label"])  # Drop the old 'label' column
            
            # Safely rename 'new_label' to 'label'
            if "new_label" in all_df.columns:
                all_df = all_df.rename(columns={"new_label": "labels"})
                logger.info("'new_label' has been renamed to 'labels'.")
            else:
                logger.warning("'new_label' column does not exist. Skipping renaming step.")
            
            # Check for 'label' column existence
            if "labels" not in all_df.columns:
                raise ValueError("The 'labels' column does not exist in all_df after renaming operations.")
            
            # Log the DataFrame columns for debugging
            print("Final DataFrame columns:", all_df.columns)
            
            # Convert to HuggingFace Dataset
            all_dataset = Dataset.from_pandas(all_df)

            tokenized_all_dataset = all_dataset.map(tokenize_function, batched=True, remove_columns=["text"])
